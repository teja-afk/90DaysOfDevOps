...
jobs:
  ...
  print-date:
    ...
    steps:
      - name: Show date
        run: echo "Date is ${{ needs.get-date.outputs.today }}"
```
...
- Useful for version numbers, build artifacts, timestamps, or dynamic values needed later in the pipeline.
---
    ...
    steps:
      - name: Branch info
        run: |
          ...
          echo "Branch: ${{ github.event.head_commit.message }}"

      - name: Check main or feature branch
        run: |
