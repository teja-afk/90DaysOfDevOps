...
In Kubernetes, Pods are ephemeral and their IP addresses are not stable. When a Pod is recreated, it gets a new IP. Also, a Deployment runs multiple Pods, so directly communicating with Pods becomes unreliable.
...
📌 Explanation:
- Exposes service on <NodeIP>:30080
...
```
DNS allows Pods to communicate without knowing IPs.
...
📌 If selector doesn't match → Endpoints will be empty → Service won’t work
...
- DNS simplifies communication
- LoadBalancer shows <pending> in local clusters

🔹 Conclusion

Kubernetes Services are essential for:
- Stable networking
- Load balancing
- Service discovery

They bridge the gap between dynamic Pods and reliable communication.
